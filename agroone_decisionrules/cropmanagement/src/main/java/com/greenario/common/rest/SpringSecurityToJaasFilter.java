...
import com.greenario.common.api.security.GroupPrincipal;
import com.greenario.common.api.security.UserPrincipal;
...
public class SpringSecurityToJaasFilter extends GenericFilterBean {
   ...
   *
   * <p>
   * The default implementation attempts to obtain the <code>Subject</code> from
   * the <code>
   * SecurityContext</code>'s <code>Authentication</code>. If it is of type <code>
   * JaasAuthenticationToken</code> and is authenticated, the <code>Subject</code>
   * is returned from
   * it.
   *
  ...
  private void initializeGroupPrincipalsFromAuthentication(
    ...
    authentication
        .getAuthorities()
        .forEach(
            grantedAuthority -> subject.getPrincipals().add(new GroupPrincipal(grantedAuthority.getAuthority())));
