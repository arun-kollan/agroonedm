...
public class QuerySortParameter<Q extends BaseQuery<?, ?>, S extends QuerySortBy<Q>>
  ...
  // the javadoc comment for this field is above its getter. This is done to
  // define the type
  // parameter S by overriding that getter and allowing spring-auto-rest-docs to
  // properly detect
   ...
   * 'sort-by'. The amount
   * of sort-by and order declarations have to match. Alternatively the value can
   * be omitted. If
   * done so the default sort order (ASCENDING) will be applied to every sort-by
   * value.
   */
  @JsonProperty("order")
  private final List<SortDirection> order;

  // this is only necessary because spring-auto-rest-docs can't resolve Enum[]
  // data types.
  ...
  public Void apply(Q query) {
    if (sortBy != null) {
      for (int i = 0; i < sortBy.size(); i++) {
        SortDirection sortDirection = order == null || order.isEmpty() ? SortDirection.ASCENDING : order.get(i);
  ...
  // this method is only static because there exists no query for the project
  // comment entity
